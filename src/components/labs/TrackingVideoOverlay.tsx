...
export default function TrackingVideoOverlay({ frames, time, showSkeleton, isProcessing = false }: { frames: TrackingTelemetryV1[]; time: number; showSkeleton: boolean; isProcessing?: boolean }) {
  ...
  return <svg aria-label="Detected players and body skeletons" viewBox="0 0 100 100" preserveAspectRatio="none" className="absolute inset-0 w-full h-full pointer-events-none">
    {frame?.players.filter(p => p.state !== 'lost').map(p => <g key={p.playerId}>
