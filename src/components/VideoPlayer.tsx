...
import { Play, Pause, Rewind, FastForward, Upload, Youtube, Settings2, MonitorPlay } from 'lucide-react';
import { formatPreciseTime } from '../utils';
import ScoutHUDMode from './hud/ScoutHUDMode';
...
export default function VideoPlayer() {
  ...
  const getInternalPlayer = useCallback(() => getPlayer()?.getInternalPlayer?.(), [getPlayer]);
  ...
  const getCurrentTimeSafe = useCallback((): number => {
    ...
    if (typeof internal?.getCurrentTime === 'function') return internal.getCurrentTime();
  ...
  }, [getCurrentTimeSafe, getCurrentTimeRef]);

  const getDurationSafe = useCallback((): number => {
    const player = getPlayer();
    const internal = getInternalPlayer();

    if (typeof player?.duration === 'number' && Number.isFinite(player.duration)) return player.duration;
    if (typeof player?.getDuration === 'function') return player.getDuration();
    if (typeof internal?.getDuration === 'function') return internal.getDuration();

    return duration || 0;
  }, [getPlayer, getInternalPlayer, duration]);

  const seekToSafe = useCallback((seconds: number) => {
    const player = getPlayer();
    const internal = getInternalPlayer();

    const maxDuration = getDurationSafe();
    const safeTarget = Math.max(0, maxDuration ? Math.min(seconds, maxDuration) : seconds);
  ...
  const seekBySafe = useCallback((delta: number) => {
  ...
  const setVolumeSafe = useCallback((nextVolume: number) => {
  ...
  }, [seekRequest, setSeekRequest, seekToSafe, playSafe, settings.autoPlayAfterSeek]);
  ...
  const extractYouTubeId = (input: string): string | null => {
    ...
    try {
      ...
      if (url.hostname.includes('youtube.com')) {
        ...
        const id =
          embedIndex !== -1 ? parts[embedIndex + 1] :
          shortsIndex !== -1 ? parts[shortsIndex + 1] :
          liveIndex !== -1 ? parts[liveIndex + 1] : null;
        return id && /^[a-zA-Z0-9_-]{11}$/.test(id) ? id : null;
      }
    } catch { return null; }
    return null;
  ...
  const handlePlayerError = (error: any) => {
    ...
    if (error && typeof error === 'number') {
      ...
      if (error === 100) {
        ...
        type = 'embed_disabled';
        message = 'เจ้าของวิดีโอไม่อนุญาตให้ฝังเล่นบนเว็บไซต์อื่นภายนอก YouTube';
      } else if (error === 153) {
        type = 'preview_iframe_restricted';
        message = 'ไม่พบสิทธิ์ Referer หรือ client identity ใน Preview iframe นี้';
      } else {
        type = 'unknown';
      ...
      message = 'ไม่สามารถเล่น YouTube นี้ได้ อาจเกิดจากคลิปไม่อนุญาตให้ฝัง, คลิปเป็น private/removed, browser block third-party embed, หรือเปิดผ่าน preview iframe ที่จำกัดสิทธิ์';
    }
  ...
  }, [isPlaying, isScrubbing, videoSourceType, youtubeUrl, videoSrc, getCurrentTimeSafe, getDurationSafe, setVideoTime]);
  ...
  }, [isPlaying, videoSrc, youtubeUrl, videoSourceType, playbackRate, seekBySafe, setSpeedSafe]);

  // Gestures
  const [gestureStart, setGestureStart] = useState<{ x: number, y: number, time: number, vol: number, bright: number } | null>(null);
  ...
  const handleVideoPointerMove = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (Math.abs(dx) > Math.abs(dy) && Math.abs(dx) > 10) {
      ...
      showGestureOverlayText(`${deltaSec > 0 ? '+' : ''}${deltaSec.toFixed(1)}s`);
      ...
      if (isRightSide) {
        ...
        const newBright = Math.max(0.5, Math.min(2, gestureStart.bright + delta));
  ...
  const showGestureOverlayText = (text: string) => {
    ...
    overlayTimerRef.current = window.setTimeout(() => setGestureOverlayText(null), 800);
  };
  ...
  const hasVideo = (videoSourceType === 'local' && videoSrc) || (videoSourceType === 'youtube' && youtubeUrl);
  const visibleTime = draftSeekTime ?? currentTimeDisplay;
      ...
      <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-col gap-1">
            ...
            {localFileName && !videoSrc && (
              <div className="text-xs text-orange-500 bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 p-2 rounded-lg text-center">
                Project นี้มีการบันทึกไฟล์ <b>{localFileName}</b><br/>
        ...
        {videoSourceType === 'youtube' && (
          <div className="flex flex-col gap-1">
            <div className="flex gap-2">
              <div className="relative flex-1">
                ...
                <input 
                  ...
                  className="w-full pl-8 pr-3 py-1.5 text-xs bg-gray-50 dark:bg-gray-900 border border-gray-300 dark:border-gray-700 rounded-lg text-gray-800 dark:text-gray-200"
                />
              </div>
              <button 
            ...
            <p className="text-[10px] text-gray-500 italic">YouTube อาจ seek ไม่ได้ละเอียดเท่า local video หากต้องการความแม่นยำสูงแนะนำให้ใช้ Local Video</p>
          </div>
        )}
      </div>
      ...
      {hasVideo ? (
        ...
        >
          ...
          >
            ...
            <div 
              ...
              style={{ 
                height: isHUDMode ? '100%' : (videoHeight === 'auto' ? 'auto' : `${videoHeight}px`),
                aspectRatio: isHUDMode ? 'auto' : (videoHeight === 'auto' ? '16/9' : 'auto'),
                minHeight: '150px',
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-gray-950/95 border border-red-500/30 z-30 p-6 text-center overflow-y-auto backdrop-blur-md">
                <div className="max-w-md w-full space-y-4">
                  ...
                  <div className="bg-black/60 border border-white/10 rounded-xl p-3.5 text-left font-mono text-[10px] sm:text-xs text-white/80 space-y-1.5 shadow-inner">
                    <div className="text-[11px] font-bold text-sky-400 border-b border-white/5 pb-1 mb-1.5 uppercase tracking-wider">
                      Diagnostics Information
                    </div>
                    <div><span className="text-white/40">Video ID:</span> <span className="text-sky-300 font-bold">{youtubeVideoId || 'Unknown'}</span></div>
                    <div className="truncate"><span className="text-white/40">URL:</span> <span className="text-white/60 text-[9px]">{youtubeUrl || 'Unknown'}</span></div>
                    <div><span className="text-white/40">Error Type:</span> <span className="text-amber-400 font-semibold">{playerErrorType || 'unknown'}</span></div>
                    <div><span className="text-white/40">Error Code:</span> <span className="text-white font-semibold">{playerErrorCode !== null ? playerErrorCode : 'None'}</span></div>
                    <div><span className="text-white/40">Browser Mode:</span> <span className="text-white/60">{window.isSecureContext ? 'Secure (HTTPS/Localhost)' : 'Insecure'}</span></div>
                    <div><span className="text-white/40">Iframe Context:</span> <span className="text-white/60">{window.self !== window.top ? 'Yes (Iframe Restricted)' : 'No (Direct Tab)'}</span></div>
                  </div>

                  {/* Action Buttons Row */}
                  <div className="grid grid-cols-2 gap-2 justify-center pointer-events-auto">
                    {videoSourceType === 'youtube' && (
                      <>
                        <button 
                          onClick={() => handleYoutubeLoad()}
            ...
            <Suspense fallback={<div className="w-full h-full flex items-center justify-center text-gray-500">Loading player...</div>}>
              {(() => {
                const currentVideoSrc = videoSourceType === 'local' ? videoSrc ?? undefined : youtubeUrl || undefined;
              ...
              <div className="flex flex-col w-full z-30 bg-gray-900 border-t border-gray-800 p-3 relative">
                ...
                {isScrubbing && duration > 0 && (
                  <div 
                    ...
                    style={{ left: `calc(4rem + 0.75rem + ${(Math.min(visibleTime, duration) / duration) * 100}% * calc(100% - 8rem - 1.5rem))` }}
                  >
                  ...
                  <div className="relative flex-1 h-8 flex items-center group">
                    ...
                    <div className="absolute left-0 right-0 h-2 bg-gray-700 rounded-full overflow-hidden pointer-events-none">
                      <div 
                        ...
                        style={{ width: `${duration > 0 ? (visibleTime / duration) * 100 : 0}%` }}
                      />
                    </div>
                    {/* Custom Thumb */}
                    <div 
                      ...
                      style={{ left: `${duration > 0 ? (Math.min(visibleTime, duration) / duration) * 100 : 0}%` }}
                    />
                    <input 
                      type="range" 
                      min={0} 
                      max={duration || 0} 
                      step={0.01}
                      value={Math.min(visibleTime, duration || visibleTime)}
        ...
        <div className="bg-gray-100 dark:bg-gray-900 aspect-video rounded-lg flex items-center justify-center border border-dashed border-gray-300 dark:border-gray-700">
          <p className="text-gray-400 dark:text-gray-500 text-sm text-center px-4">
            {videoSourceType === 'local' ? 'ยังไม่ได้เลือกไฟล์วิดีโอ' : 'ยังไม่ได้ใส่ URL YouTube'}<br/>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-2">
            <div className="flex items-center justify-center gap-2 sm:gap-4 relative">
              <button onClick={() => seekBySafe(-3)} className="p-2 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700" title="-3s (Left)">
                <Rewind size={18} />
                <span className="text-[10px] block -mt-1">-3s</span>
              </button>
              <button onClick={() => seekBySafe(-1)} className="p-2 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700" title="-1s (Alt+Left)">
                <span className="text-xs font-bold block">-1s</span>
              </button>
              
              <button onClick={togglePlay} className="p-4 rounded-full bg-sky-600 text-white hover:bg-sky-700 shadow-md mx-2" title="Play/Pause (Space)">
                {isPlaying ? <Pause size={24} /> : <Play size={24} />}
              </button>
              
              <button onClick={() => seekBySafe(1)} className="p-2 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700" title="+1s (Alt+Right)">
                <span className="text-xs font-bold block">+1s</span>
              </button>
              <button onClick={() => seekBySafe(3)} className="p-2 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700" title="+3s (Right)">
                <FastForward size={18} />
                <span className="text-[10px] block -mt-1">+3s</span>
              </button>
              ...
              <div className="absolute right-0 flex items-center gap-1">
                {settings.enableScoutHUDMode !== false && (
                  <button 
                    ...
                    className="p-2 rounded-full bg-sky-100 text-sky-600 dark:bg-sky-900/50 dark:text-sky-400 hover:bg-sky-200 dark:hover:bg-sky-800"
                    title="Scout HUD Mode"
                  >
                    <MonitorPlay size={16} />
                  </button>
                )}
            ...
            {showFineControls && (
              <div className="flex items-center justify-center gap-2 mt-2 pt-2 border-t border-gray-100 dark:border-gray-800">
                <button onClick={() => seekBySafe(-5)} className="px-3 py-1 rounded bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 text-xs hover:bg-gray-200">-5s</button>
                <button onClick={() => seekBySafe(-0.1)} className="px-3 py-1 rounded bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 text-xs hover:bg-gray-200">-0.1s</button>
                <button onClick={() => seekBySafe(0.1)} className="px-3 py-1 rounded bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 text-xs hover:bg-gray-200">+0.1s</button>
                <button onClick={() => seekBySafe(5)} className="px-3 py-1 rounded bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 text-xs hover:bg-gray-200">+5s</button>
              </div>
            )}
