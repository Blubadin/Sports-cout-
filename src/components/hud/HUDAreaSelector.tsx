...
export default function HUDAreaSelector({ isActive, onPointerDown, onPointerUp, onClick, onClose }: Props) {
  const { currentAction, setCurrentAction, setCurrentInputHistory, sportTemplate, matchInfo, settings, teams } = useScoutContext();
  const layout = useHUDDeviceLayout();

  const handleSelectArea = (code: string, courtSide?: 'teamA' | 'teamB' | 'neutral') => {
    const previousAreaCode = currentAction.areaCode;
    const previousCourtSide = currentAction.courtSide;
    
    setCurrentInputHistory(hist => [...hist, { 
      type: 'field', 
      category: 'areaCode', 
      previousValue: previousAreaCode, 
      value: code,
      previousCourtSide,
      courtSide
    }]);

    setCurrentAction(prev => ({ ...prev, areaCode: code, courtSide }));
  ...
  const panelStyle = isActive ? {
    width: layout.device === 'mobile' 
      ? (layout.orientation === 'portrait' ? '45vw' : '40vw') 
      : '320px',
    ...
    >
      ...
      >
        <div className={`w-full flex justify-between items-center mb-1`}>
          <div className={`text-white/50 font-bold uppercase tracking-wider ${isActive ? 'text-[10px] sm:text-xs' : 'text-[9px]'} pl-1`}>
            Area (W)
          </div>
          {isActive && onClose && (
            <button onClick={(e) => { e.stopPropagation(); onClose(); }} className="p-1 bg-white/10 rounded-full hover:bg-white/20">
              <X size={12} className="text-white/70" />
            </button>
          )}
        </div>
        
        <div className={`overflow-auto w-full ${isActive ? 'scrollbar-none flex justify-center' : ''}`}>
