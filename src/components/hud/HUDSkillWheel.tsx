...
export default function HUDSkillWheel({
  ...
  const maxDescriptorGroups = selectedSkill && descriptors ? descriptors.length : 0;
  const paddingMultiplier = maxDescriptorGroups > 0 ? 1 + (maxDescriptorGroups * 0.22) : 1;
  const actualSize = size * paddingMultiplier;
  const center = actualSize / 2;
  const radius = size * 0.22; // Base radius for main skills based on original size
  ...
  if (!active || compact) {
    return (
      <div className="flex flex-col items-center justify-center">
        ...
        <span className="text-[10px] text-white/50 font-bold">(Q)</span>
      </div>
    );
  }

  // Draw radial logic
  const renderItems = (items: { code: string; label: string }[], customRadius?: number, isDescriptor = false, groupId?: string) => {
          ...
          onPointerEnter={() => {
            if (active) {
              if (isDescriptor && groupId) {
                ...
                (window as any).__hoveredDescriptor = { groupId, optionCode: item.code };
              } else {
        ...
        >
          <circle 
            ...
            className={`transition-all duration-200 ${
              isSelected 
                ? (isDescriptor ? 'fill-indigo-500 stroke-white' : 'fill-cyan-500 stroke-white')
          ...
          <text 
            ...
            className={`font-bold transition-all pointer-events-none select-none ${
              isDescriptor ? 'text-[9px] sm:text-[11px]' : 'text-[11px] sm:text-xs'
            } ${
              isSelected ? 'fill-white' : 'fill-white/80 group-hover:fill-white'
            }`}
          >
            {item.label}
          </text>
  ...
  return (
    <div className="scout-skill-menu relative pointer-events-auto transition-all duration-300" style={{ width: actualSize, height: actualSize }}>
      <svg width="100%" height="100%" viewBox={`0 0 ${actualSize} ${actualSize}`} className="absolute inset-0 overflow-visible">
        ...
        {skills.map((skill, index) => {
          ...
          return (
            <line 
              ...
              className="stroke-white/10" strokeWidth="2" strokeDasharray="4 4"
            />
          );
        })}
