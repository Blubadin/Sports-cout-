...
describe('Phase 0.5 tracking analytics honesty', () => {
  ...
  it('preserves real quality values and labels measured cadence separately', () => {
    render(<BadmintonMovementDashboard analysis={analysis()} samples={[measuredSample]} />);

    expect(screen.getByText('91.0%')).toBeInTheDocument();
    expect(screen.getAllByText('82%').length).toBeGreaterThan(0);
    expect(screen.getByText('9.0%')).toBeInTheDocument();
    const cadence = screen.getByTestId('telemetry-cadence-card');
    expect(cadence).toHaveTextContent('Source: 30 FPS');
    expect(cadence).toHaveTextContent('Analyzed: 15 Hz');
    expect(cadence).toHaveTextContent('Stored: 15 Hz');
  ...
  it('does not render null quality metrics from an older saved session as zero', async () => {
